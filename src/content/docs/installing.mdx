...
`codeanalyzer-typescript` is distributed as source on the [GitHub repository](https://github.com/codellm-devkit/codeanalyzer-ts). You build it once with [Bun](https://bun.sh/) into a standalone native binary, then run that binary anywhere.
...
Level-2 analysis (`--analysis-level 2`) is designed to enrich the call graph with CodeQL. You do **not** need to install anything for it today — it is an [experimental, not-yet-implemented stub](/codeanalyzer-ts/guides/level-2/) that currently falls back to the level-1 graph.
